 ...
 *   - TOP ROW: Entf (DEL) | Einfg (INS) | Druck (PSCR)
...
[_BL] = LAYOUT(
  KC_ESC,   KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,    KC_F6,    KC_F7,    KC_F8,    KC_F9,    KC_F10,   KC_F11,   KC_F12,   KC_DEL,   CUS_INS,  KC_PSCR,  KC_PGUP,  KC_PGDN,
    ...
    if (record->event.pressed) {
        switch (keycode) {
            ...
            case KC_CAPS:
                ...
                break;
            case KC_NUM:
                if (led != NO_LED) start_pulse(&num_pulse, led, !host_keyboard_led_state().num_lock);
                break;
            case CUS_INS:
                ins_press_time = timer_read32(); ins_key_held = true;
        ...
        if (keycode == CUS_INS && ins_key_held) {
            ...
            if (timer_elapsed32(ins_press_time) >= INS_LONG_PRESS_MS) {
                insert_mode_on = true;
...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    { uint8_t nl = g_led_config.matrix_co[4][6];
      if (!num_pulse.active && nl != NO_LED && nl >= led_min && nl < led_max && !host_keyboard_led_state().num_lock)
          rgb_matrix_set_color(nl, 0, 0, 0);
    }
    if (!ins_pulse.active && !insert_mode_on) {
        uint8_t il = ins_pulse.led_idx;
        if (il != NO_LED && il >= led_min && il < led_max) rgb_matrix_set_color(il, 0, 0, 0);
    }
    ...
    for (uint8_t p = 0; p < 3; p++) {
        ...
        if (pl >= led_min && pl < led_max) {
            rgb_matrix_set_color(pl, pulses[p]->is_green ? 0 : br, pulses[p]->is_green ? br : 0, 0);
        }
    }
